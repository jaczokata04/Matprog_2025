...
def play_hand(deck, player_hand, dealer_hand, bet, is_split_hand=False):
    ...
    while True:
        ...
        # Ellenőrzés, hogy normál vagy összetett eredményt kaptunk-e
        if isinstance(strategy_result, tuple):
            modified_strategy, basic_strategy = strategy_result
            print(f"Stratégia ajánlás: {interpret_strategy(modified_strategy)} (alap stratégia: {interpret_strategy(basic_strategy)})")
            print(f"A true count ({true_count}) módosította az alap stratégiát!")
            strategy = modified_strategy
        else:
            strategy = strategy_result
            print(f"Stratégia ajánlás: {interpret_strategy(strategy)}")
